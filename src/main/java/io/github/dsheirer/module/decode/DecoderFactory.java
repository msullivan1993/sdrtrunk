...
public class DecoderFactory
{
                                       ...
                                       UserPreferences userPreferences)
    {
        ...
        modules.add(new AudioModule(aliasList, AbstractAudioModule.DEFAULT_TIMESLOT, maxAudioSegmentLengthMillis, AUDIO_FILTER_ENABLE, false, 0, true));
    ...
    private static void processNBFM(Channel channel, List<Module> modules, AliasList aliasList, DecodeConfiguration decodeConfig)
    {
        ...
        modules.add(new AudioModule(aliasList, 0, 60000, decodeConfigNBFM.isAudioFilter(), decodeConfigNBFM.isRequireAliasMatch(), decodeConfigNBFM.getSquelchDelayTimeMs(), decodeConfigNBFM.isSquelchDelayRemoveSilence()));
    ...
    private static void processAM(Channel channel, List<Module> modules, AliasList aliasList, DecodeConfiguration decodeConfig)
    {
        if(decodeConfig instanceof DecodeConfigAM configAM)
        {
            ...
            modules.add(new AudioModule(aliasList, 0, 60000, AUDIO_FILTER_ENABLE, false, 0, true));
        }
        else
        {
